...
public class Rect extends RenderableEntity {
    ...
    public static void draw(double x, double y, double x2, double y2, int color, RectType type) {
        if (type == RectType.EXPAND) {
            RenderUtils.drawRect(x, y, x + x2, y + y2, color);
    ...
    public void draw() {
        ...
        if (this.getRectType() == RectType.EXPAND) {
            RenderUtils.drawRect(this.getX(), this.getY(), this.getX() + this.getWidth(), this.getY() + this.getHeight(), this.getColor());
        } else if (this.getRectType() == RectType.ABSOLUTE_POSITION) {
            RenderUtils.drawRect(this.getX(), this.getY(), this.getWidth(), this.getHeight(), this.getColor());
        }
    }
