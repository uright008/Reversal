...
public class Interpolations {
    ...
    public static float interpBezier(float startValue, float endValue,
        ...
        double result = startValue + (endValue - startValue) * interpolateCurve(fraction) * RenderUtil.deltaFrameTime * 100;
    ...
    public static void calcFrameDelta() {
        ...
        RenderUtil.deltaFrameTime = (float) (((double) System.nanoTime() - (double) lastNanoFrame) / 1000000000.0);
        lastNanoFrame = System.nanoTime();
    }

    public static Color getColorAnimationState(Color animation, Color finalState, double speed) {
        float add = (float) (RenderUtil.deltaFrameTime * 10 * speed);
    ...
    public static float interpLinear(float now, float end, float interpolation) {
        float add = (float) (RenderUtil.deltaFrameTime * 10 * interpolation);
        if (now < end) {
            if (now + add < end)
                now += add;
            else
