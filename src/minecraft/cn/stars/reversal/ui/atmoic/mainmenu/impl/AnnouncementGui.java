...
public class AnnouncementGui extends AtomicGui {
    ...
    private void loadAnnouncements() {
        ...
        Announcement announcement2 = new Announcement("Reversal 3.0.0-Insider", "2025/4/27", 1)
                ...
                .addContent("[+] AtomicIsland新增自定义进度条和发光颜色").mark()
                .addContent("[+] ClientSettings新增Shader渐变边框, 新增编辑自定义名称选项").mark()
                .addContent("[+] 新增TextValue - 编辑文本")
                .addContent("[+] 新增CustomValue - 执行自定义代码")
                .addContent("[+] 新增随机标题").mark()
                .addContent("[/] 优化ServerConnector").mark()
                .addContent("[/] 优化ServerPinger").mark()
                .addContent("[/] 优化CustomName").mark()
                .addContent("[/] 掉线界面显示时间和当前跨版本")
                .addContent("[/] 第一次启动时自动开启HUD").mark()
                .addContent("[/] 优化日志系统").mark()
                .addContent("[*] 修复部分Kotlin导致的问题").mark()
                .addContent("[*] 修复主菜单头像无法获取").mark()
                .addContent("[-] 删除.name指令").mark()
                .calcMaxWidth();
        announcements.clear();
