...
public class FBPParticleSnow extends EntityDiggingFX {
	...
	public FBPParticleSnow(World worldIn, double xCoordIn, double yCoordIn, double zCoordIn, double xSpeedIn,
		...
		try {
			this.setSourcePos(new BlockPos(xCoordIn, yCoordIn, zCoordIn));
		} catch (Throwable e1) {
			e1.printStackTrace();
		}
		...
		if (timeTraveller.enabled) {
			if (weather.equals("Light Snow")) {
				particleScale *= FBP.random.nextDouble(FBP.scaleMult - 1f, FBP.scaleMult - 0.2f);
				particleMaxAge = (int) FBP.random.nextDouble(100, 200);
				...
				particleScale *= FBP.random.nextDouble(FBP.scaleMult - 1.5f, FBP.scaleMult - 0.5f);
				particleMaxAge = (int) FBP.random.nextDouble(50, 100);
				this.particleRed = 0.85f;
				this.particleGreen = 0.78f;
				this.particleBlue = 0.94f;
			}
		} else {
			particleScale *= FBP.random.nextDouble(FBP.scaleMult - 0.25f, FBP.scaleMult + 0.25f);
			particleMaxAge = (int) FBP.random.nextDouble(250, 300);
