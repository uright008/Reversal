		...
		if (f > 0.0F) {
			...
			for (int k1 = k - i1; k1 <= k + i1; ++k1) {
				for (int l1 = i - i1; l1 <= i + i1; ++l1) {
					...
					if (biomegenbase.getEnableSnow() || biomegenbase.canRain()) {
						...
						if (k2 != l2) {
							this.random
									.setSeed(l1 * l1 * 3121 + l1 * 45238971 ^ k1 * k1 * 418711 + k1 * 13761);
							...
							if (world.getWorldChunkManager().getTemperatureAtHeight(f2, j2) >= 0.15F && (!timeTraveller.isEnabled() || !(weather.equals("Snow") || weather.equals("Light Snow") || weather.equals("Nether")))) {
								if (!FBP.fancyRain) {
									if (j1 != 0) {
										if (j1 >= 0) {
											tessellator.draw();
								...
								int k4 = i4 & 65535;
								worldrenderer.pos((double) l1 - d3 + 0.5D, (double) k2, (double) k1 - d4 + 0.5D)
										.tex(0.0D + d9, (double) k2 * 0.25D + d8 + d10).color(color.getRed() / 255F, color.getGreen() / 255F, color.getBlue() / 255F, f5)
										.lightmap(j4, k4).endVertex();
								worldrenderer.pos((double) l1 + d3 + 0.5D, (double) k2, (double) k1 + d4 + 0.5D)
										.tex(1.0D + d9, (double) k2 * 0.25D + d8 + d10).color(color.getRed() / 255F, color.getGreen() / 255F, color.getBlue() / 255F, f5)
										.lightmap(j4, k4).endVertex();
								worldrenderer.pos((double) l1 + d3 + 0.5D, (double) l2, (double) k1 + d4 + 0.5D)
										.tex(1.0D + d9, (double) l2 * 0.25D + d8 + d10).color(color.getRed() / 255F, color.getGreen() / 255F, color.getBlue() / 255F, f5)
										.lightmap(j4, k4).endVertex();
								worldrenderer.pos((double) l1 - d3 + 0.5D, (double) l2, (double) k1 - d4 + 0.5D)
										.tex(0.0D + d9, (double) l2 * 0.25D + d8 + d10).color(color.getRed() / 255F, color.getGreen() / 255F, color.getBlue() / 255F, f5)
										.lightmap(j4, k4).endVertex();
							}
						}
					}
			...
			if (f > 0.0F) {
				if (tickCounter++ >= 2) {
					...
					for (int i = 0; i < 8 * FBP.weatherParticleDensity; i++) {
						...
						if (biome.getEnableSnow() || biome.canRain()) {
